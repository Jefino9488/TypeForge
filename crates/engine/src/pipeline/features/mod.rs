...
impl FeatureExtractor for BasicFeatureExtractor {
    fn extract(&self, request: &PredictionRequest, candidate: &RawCandidate, features: &mut FeatureSet) {
        ...
        let user_freq = self.learner.learning_db.get_weight(&candidate.text, None).unwrap_or(0);
        features.user_frequency = (user_freq as f32 / 1000.0).min(1.0); // Assuming 1000 uses is "max" weight

        // Context frequency
        if !request.application.is_empty() {
            let ctx_freq = self.learner.learning_db.get_weight(&candidate.text, Some(&request.application)).unwrap_or(0);
            features.context_score = (ctx_freq as f32 / 500.0).min(1.0);
        }

        // Session frequency
        features.session_frequency = if self.learner.session_memory.contains(&candidate.text) { 1.0 } else { 0.0 };
