...
fcitx::AddonInstance *
TypeForgeEngineFactory::create(fcitx::AddonManager *manager) {
...
TypeForgeEngine::TypeForgeEngine(fcitx::Instance *instance)
  ...
  if (pipe(pipe_fd_) == 0) {
    ...
    io_event_ = instance->eventLoop().addIOEvent(
        pipe_fd_[0], fcitx::IOEventFlag::In,
...
void TypeForgeEngine::reset(const fcitx::InputMethodEntry &,
...
void TypeForgeEngine::keyEvent(const fcitx::InputMethodEntry &,
  ...
  if (key.isLAZ() || key.isUAZ()) {
    ...
    FCITX_INFO() << "Requesting prediction for: " << preedit_
                 << " surrounding: " << surrounding << " generation "
                 << current_generation_;
    typeforge_predict_async(preedit_.c_str(), surrounding.c_str(),
                            app.empty() ? nullptr : app.c_str(),
                            current_generation_,
                            TypeForgeEngine::onPredictionsReady, this);
    keyEvent.filterAndAccept();
    return;
  }
  ...
  if (!preedit_.empty()) {
    ...
    if (key.sym() == FcitxKey_Return || key.sym() == FcitxKey_space) {
      ...
      if (key.sym() == FcitxKey_space) {
        should_commit_candidate = true;
                 ...
                 suggestion_explicitly_selected_) {
        should_commit_candidate = true;
      }
      ...
      if (should_commit_candidate) {
        if (candidateList && candidateList->size() > 0) {
          ...
          std::string app = ic->program();
          std::string surrounding = ic->surroundingText().text();
          C_PredictionList *list =
              typeforge_predict_sync(preedit_.c_str(), surrounding.c_str(),
                                     app.empty() ? nullptr : app.c_str());
    ...
    // For punctuation and symbols, commit the current word and the symbol
    // explicitly.
    std::string utf8 = fcitx::Key::keySymToUTF8(key.sym());
    bool has_modifier =
        static_cast<bool>(key.states() & fcitx::KeyState::Ctrl) ||
        static_cast<bool>(key.states() & fcitx::KeyState::Alt) ||
        static_cast<bool>(key.states() & fcitx::KeyState::Super);

    if (!has_modifier && !utf8.empty() && key.sym() >= 0x20 &&
        key.sym() <= 0x7E) {
      commitString(ic, preedit_, false);
    ...
    // For control keys or non-printable, just commit preedit and let key pass
    // through to the app
    commitString(ic, preedit_, false);
    return;
  }
...
void TypeForgeEngine::commitString(fcitx::InputContext *ic,
                                   const std::string &str, bool is_accepted) {
...
void TypeForgeEngine::processResultQueue() {
  ...
  for (auto list : queue) {
    ...
    FCITX_INFO() << "Predictions ready, count: " << (list ? list->count : 0)
      ...
      for (size_t i = 0; i < list->count; ++i) {
        if (list->predictions[i].text) {
          ...
          candidateList->append(
              std::make_unique<TypeForgeCandidateWord>(txt, ic, this));
        }
      }
...
void TypeForgeEngine::onPredictionsReady(C_PredictionList *list,
                                         void *user_data) {
...
TypeForgeCandidateWord::TypeForgeCandidateWord(std::string text,
                                               fcitx::InputContext *ic,
                                               TypeForgeEngine *engine)
    : fcitx::CandidateWord(fcitx::Text(text)), text_(std::move(text)), ic_(ic),
      engine_(engine) {}
